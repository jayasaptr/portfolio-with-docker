...
const UpdateExperiencePage = () => {
  ...
  const fetchExperience = async () => {
    try {
      const response = await api.get(`/api/v1/experience/${id}`);
      ...
      setDate({
        ...
        to: new Date(experience.end_date),
      });
      setPreviewImage(experience.image);
      setSkillsSelect(experience.skills || []);
    } catch (error) {
  ...
  const handleSubmitExperience = async (data) => {
    ...
    try {
      ...
      if (response.status === 200) {
        toast("Experience has been updated successfully.", {
  ...
  const handleDeleteSkill = async (index) => {
    const skill = skillsSelect[index];
    const updatedSkills = skillsSelect.filter((_, idx) => idx !== index);
    setSkillsSelect(updatedSkills);
    const formData = new FormData();
